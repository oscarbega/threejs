...
function init() {
  ...
  const ligt = new THREE.AmbientLight(0xffffff,10)
  scene.add(ligt)
  ...
  loader.load("assets/terrains/map2.osm", function (text) {   
    ...
    for (let i = 0; i < x.length; i++) {
      ...
      for (let j = 0; j < tags.length; j++) {
        if (tags[j].hasAttribute("k")) {
         ...
         if(tags[j].getAttribute("k")=="natural" && tags[j].getAttribute("v")=="tree") {
            interest = 3;
            console.log("uwu");break;
         }
        }
      }
      ...
      if (interest > 0) {
        ...
        if (interest == 1) {
          ...
          else if (interest == 3) { //Points of interest
            console.log(points)
          }
        }
      }
    ...
    for (let i = 0; i < tags.length; i++) {
      const tag = tags[i];
      const name = tag.getAttribute("k");
      const type = tag.getAttribute("v")
      if(name=="natural" && type=="tree"){
        const parent = tag.parentNode
        console.log(parent)
        let mlat = Mapeo(parent.getAttribute("lat"),minlat,maxlat,-5,5)
        let mlon = Mapeo(parent.getAttribute("lon"),minlon,maxlon,-5,5)
        tree(mlon,mlat)
        console.log(mlon,mlat,tag.getAttribute("lat"))
      }
    }
